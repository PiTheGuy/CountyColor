...
public class RenderUtil {
    ...
    public static void drawThickPolyline(ShapeRenderer renderer, List<Vector2> points, float thickness, int size) {
        ...
        for (int i = 0; i < points.size(); i++) {
            Vector2 p1 = points.get(i).cpy().scl(size / 2f);
            Vector2 p2 = points.get((i + 1) % points.size()).cpy().scl(size / 2f);
            Vector2 direction = new Vector2(p2).sub(p1).nor();
            Vector2 perpendicular = new Vector2(-direction.y, direction.x).scl(thickness / 2f);

            Vector2 v1 = new Vector2(p1).add(perpendicular);
            Vector2 v2 = new Vector2(p1).sub(perpendicular);
            Vector2 v3 = new Vector2(p2).sub(perpendicular);
            Vector2 v4 = new Vector2(p2).add(perpendicular);
            renderer.triangle(v1.x, v1.y, v2.x, v2.y, v3.x, v3.y);
            renderer.triangle(v3.x, v3.y, v4.x, v4.y, v1.x, v1.y);
            if (lastV3 != null) {
                Vector2 prevDirection = new Vector2(lastV4).sub(lastV3).nor();
                ...
                renderer.triangle(prevOuter.x, prevOuter.y, p1.x, p1.y, outer.x, outer.y);
                renderer.triangle(prevInner.x, prevInner.y, p1.x, p1.y, inner.x, inner.y);
